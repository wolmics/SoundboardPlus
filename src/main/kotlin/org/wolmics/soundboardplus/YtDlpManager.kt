...
class YtDlpManager {
    ...
    private val isWindows = Util.getOperatingSystem().name.contains("windows", ignoreCase = true)
    ...
    fun installYtDlp(toast: TutorialToast? = null): Boolean {
        return try {
            ToastManager.updateTextAsync(toast, Text.literal("Downloading yt-dlp..."))
    ...
    fun installFfmpeg(toast: TutorialToast? = null): Boolean {
        return try {
            ToastManager.updateTextAsync(toast,Text.literal("Downloading FFmpeg..."))
    ...
    fun updateYtDlpNightly(toast: TutorialToast? = null): Boolean {
        return try {
            ToastManager.updateTextAsync(toast, Text.literal("Downloading yt-dlp (nightly)......"))
            uninstallYtDlp()
    ...
    fun downloadUrlIntoSoundDir(url: String, category: String?): Pair<Boolean, String> {
        ...
        val toast = ToastManager.createProgressToast(Text.literal("Preparing Download..."))
    ...
    private fun streamProcessOutput(proc: Process, toast: TutorialToast) {
        ToastManager.updateTextAsync(toast, Text.literal("Downloading..."))
                ...
                reader.forEachLine { line ->
                    ...
                    when {
                        ...
                        line.startsWith("[ExtractAudio]") ->
                            ToastManager.updateTextAsync(toast, Text.literal("Converting to mp3..."))
                    }
                    logger.debug(line)
                }
            }
    ...
    private fun awaitProcess(proc: Process, toast: TutorialToast): Pair<Boolean, String> {
        ...
        return if (proc.exitValue() == 0) {
            ToastManager.updateTextAsync(toast, Text.literal("Download Complete!").formatted(Formatting.BOLD).withColor(Color(0, 210, 0).rgb))
            ToastManager.hideToast(toast, 2000L)
    ...
    private fun failure(toast: TutorialToast?, msg: String): Pair<Boolean, String> {
        ToastManager.updateTextAsync(toast, Text.literal(msg).formatted(Formatting.RED))
        ToastManager.hideToast(toast, 2000L)
