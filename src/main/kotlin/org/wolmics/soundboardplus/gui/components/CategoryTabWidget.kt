...
class CategoryTabWidget(
    ...
    fun render(context: DrawContext, mouseX: Int, mouseY: Int, areaLeft: Int, areaRight: Int) {
        tabRects.clear()
        ...
        val tabWidths = defs.map { textRenderer.getWidth(it.label) + 12 }
        ...
        if (!needsPaging) {
            ...
            for ((index, def) in defs.withIndex()) {
                ...
                curX = drawTab(context, def.label, curX, tabWidths[index], def.category, active, mouseX, mouseY, name = def.name)
            }
        } else {
            // Reserve room on the right for both nav arrows and a "page X/Y" label
            val labelReserve = textRenderer.getWidth("00/00") + 10
            ...
            for (i in pages.getOrElse(currentPage) { emptyList() }) {
                val def = defs[i]
                val active = def.name != "categoryAdder" && selectedCategory == def.category
                curX = drawTab(context, def.label, curX, tabWidths[i], def.category, active, mouseX, mouseY, name = def.name)
            }

            drawPageArrow(context, areaLeft, "◀", currentPage > 0, mouseX, mouseY, "prevPage")
            drawPageArrow(context, areaRight - pageArrowWidth, "▶", currentPage < pages.size - 1, mouseX, mouseY, "nextPage")
            ...
            context.drawText(
                textRenderer, pageLabel,
                labelX + (labelW - textRenderer.getWidth(pageLabel)) / 2,
    ...
    private fun drawBorderedBox(context: DrawContext, x: Int, w: Int, bg: Int, border: Int) {
        context.fill(x, barY, x + w, barY + barHeight, bg)
        context.fill(x, barY, x + w, barY + 1, border)                     // top
        context.fill(x, barY + barHeight - 1, x + w, barY + barHeight, border) // bottom
        context.fill(x, barY, x + 1, barY + barHeight, border)               // left
        context.fill(x + w - 1, barY, x + w, barY + barHeight, border)       // right
    }
    ...
    private fun drawPageArrow(
        ...
        drawBorderedBox(context, x, w, bg, border)
        context.drawText(
            textRenderer, label,
